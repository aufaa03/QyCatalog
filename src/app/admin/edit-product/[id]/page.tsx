...
export default function EditProductPage({ params }: { params: { id: string } }) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (imageFile) {
            ...
            const { error: uploadError } = await supabase.storage.from('products').upload(fileName, imageFile, { cacheControl: '3600', upsert: false })
        ...
        if (updateError) { setError(`Gagal update produk: ${updateError.message}`); setLoading(false); return }
        ...
        return (
            <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--color-bg)' }}>
                <div style={{ textAlign: 'center', padding: '40px' }}>
                    <div className="skeleton" style={{ width: '240px', height: '240px', borderRadius: '16px', margin: '0 auto 20px' }} />
                    <div className="skeleton" style={{ height: '16px', borderRadius: '6px', marginBottom: '10px' }} />
                    <div className="skeleton" style={{ height: '16px', borderRadius: '6px', width: '60%', margin: '0 auto' }} />
                </div>
            </main>
        )
    }

    if (!product) return <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><p>Produk tidak ditemukan.</p></main>
        ...
        <main style={{ minHeight: '100vh', background: 'var(--color-bg)' }}>
            ...
            <header style={{ background: 'linear-gradient(135deg, #7C3AED 0%, #6D28D9 100%)', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 20px rgba(124,58,237,0.3)' }}>
                <div className="admin-container" style={{ padding: '14px 24px', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                        <h1 style={{ color: '#fff', fontWeight: 800, fontSize: '1.05rem' }}>Edit Produk</h1>
                        <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.7rem' }}>Ubah detail produk</p>
                    </div>
                </div>
            </header>
                    ...
                    <div style={{
                        ...
                        <div>
                            ...
                            <div
                                ...
                                onDrop={handleDrop}
                                style={{
                                    border: `2px dashed ${dragOver ? 'var(--color-primary)' : 'var(--color-secondary)'}`,
                                    borderRadius: '16px',
                                    cursor: 'pointer',
                                    overflow: 'hidden',
                                    position: 'relative',
                            ...
                            >
                                {imagePreview && <Image src={imagePreview} alt="Preview" fill style={{ objectFit: 'contain' }} />}
                                <div style={{ position: 'absolute', bottom: '12px', right: '12px', background: 'rgba(124,58,237,0.9)', color: '#fff', padding: '6px 14px', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 600, zIndex: 2 }}>
                                    Ganti Foto
                                </div>
                            </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            <div>
                                ...
                                <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} className="input-field" required placeholder="Nama produk" style={{ fontSize: '1rem', padding: '14px 16px' }} />
                            </div>
                            ...
                            <div>
                                ...
                                <div style={{ position: 'relative' }}>
                                    ...
                                    <input id="price" type="text" inputMode="numeric" value={price} onChange={(e) => setPrice(formatPriceInput(e.target.value))} className="input-field" required style={{ paddingLeft: '42px', fontSize: '1rem', padding: '14px 16px 14px 42px' }} />
                                </div>
                            </div>

                            <div>
                                <label className="label" htmlFor="url">Link Affiliate *</label>
                                <input id="url" type="url" value={affiliateUrl} onChange={(e) => setAffiliateUrl(e.target.value)} className="input-field" required placeholder="https://..." style={{ fontSize: '1rem', padding: '14px 16px' }} />
                            </div>
